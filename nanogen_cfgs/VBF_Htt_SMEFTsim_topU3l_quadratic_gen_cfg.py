...
# with command line options: Configuration/Generator/nomerge_tauola_fragment.py --python_filename VBF_Htt_SMEFTsim_topU3l_quadratic_gen_cfg.py --eventcontent GEN --datatier GEN --conditions 130X_mcRun3_2023_realistic_postBPix_v5 --beamspot Realistic25ns13p6TeVEarly2023Collision --step LHE,GEN --geometry DB:Extended --era Run3_2023 --customise Configuration/DataProcessing/Utils.addMonitoring --fileout file:gen.root --no_exec --mc -n 5000
...
process.configurationMetadata = cms.untracked.PSet(
    annotation = cms.untracked.string('Configuration/Generator/nomerge_tauola_fragment.py nevts:5000'),
...
# Schedule definition
process.schedule = cms.Schedule(process.lhe_step,process.generation_step,process.genfiltersummary_step,process.endjob_step,process.GENoutput_step)
...
if hasattr(process, "LHCTransport"):
    lhct = process.LHCTransport

    for pname in process.paths_():
        path = getattr(process, pname)
        if "LHCTransport" in path.moduleNames():
            path.remove(lhct)

    for ename in process.endpaths_():
        ep = getattr(process, ename)
        if "LHCTransport" in ep.moduleNames():
            ep.remove(lhct)

    for tname in process.tasks_():
        task = getattr(process, tname)
        if "LHCTransport" in task.moduleNames():
            task.remove(lhct)

    delattr(process, "LHCTransport")

if hasattr(process, "RandomNumberGeneratorService"):
    rng = process.RandomNumberGeneratorService
    if hasattr(rng, "LHCTransport"):
        delattr(rng, "LHCTransport")